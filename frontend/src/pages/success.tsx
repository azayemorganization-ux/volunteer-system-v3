...
export interface VolunteerCardData {
  ...
  fullName: string;
  phone: string;
  unitId: number; // الرقم القادم من الأسكيما الرسمية
...
// دالة معالجة وعرض اسم الوحدة ذكياً لمنع ظهور "غير محدد"
const resolveUnitName = (volunteer: VolunteerCardData) => {
  ...
  if (volunteer.unitId) return `الوحدة الميدانية المعتمدة`; // معالجة ذكية للرقم القادم من الأسكيما
  return "مكتب الطوارئ المركزي";
};

/* ----------------------------------------------------
  1. مكون البطاقة الرقمية الرسمية (نظيف وجاهز للاسكرين شوت)
---------------------------------------------------- */
export function IDCard({ volunteer }: { volunteer: VolunteerCardData }) {
  ...
  return (
    ...
    >
      ...
      {/* الصورة الشخصية أو الحروف الملكية */}
        ...
        <div className="relative w-28 h-28 rounded-full border-[5px] border-white shadow-xl p-0.5 bg-gradient-to-tr from-red-600 via-red-500 to-red-400 overflow-hidden mx-auto">
          <div className="w-full h-full rounded-full border border-white/20 overflow-hidden bg-white">
            {volunteer.photoUrl ? (
              ...
              <div className="w-full h-full bg-[#991b1b] text-white flex items-center justify-center font-black text-xl tracking-wider">
      ...
      <div className="relative z-10 w-full bg-white/95 backdrop-blur-md rounded-[2.5rem] border border-red-50 shadow-lg p-5 mb-3 overflow-hidden border-b-[4px] border-b-red-50/50">
        ...
        <div className="relative z-20 flex flex-row-reverse items-start justify-between border-t border-gray-100 pt-4">
          ...
          <div className="grid grid-cols-1 gap-y-3 text-right flex-1">
            <div>
              <div className="text-[8px] text-gray-400 font-bold mb-0.5">رقم القيد المتطوع</div>
              <div className="text-[11px] font-black text-red-600 font-mono tracking-tight">{volunteer.volunteerId}</div>
            </div>
            <div>
              <div className="text-[8px] text-gray-400 font-bold mb-0.5">الوحدة الميدانية</div>
              <div className="text-[10px] font-black text-gray-700 truncate">{resolveUnitName(volunteer)}</div>
            </div>
          </div>
        </div>
...
/* ----------------------------------------------------
  3. لوحة أدوات الرفاهية التفاعلية الخارجية (خارج الكارنيه)
---------------------------------------------------- */
function ActionControlPanel({ volunteerId, whatsappMessage }: { volunteerId: string, whatsappMessage: string }) {
  ...
  return (
    <div className="w-full max-w-sm bg-white/95 backdrop-blur-md border border-red-900/20 rounded-[2rem] p-4 shadow-xl flex flex-col space-y-3 mt-1">
      <div className="flex items-center justify-between bg-slate-50 px-4 py-2.5 rounded-xl border border-slate-100">
        <div className="text-right">
          <p className="text-[9px] text-slate-400 font-bold">رقم القيد الرسمي</p>
...
/* ----------------------------------------------------
  4. لوحة تتبع المسار الميداني مع الخط الرابط والعبارات السهلة
---------------------------------------------------- */
function InstitutionalStatusStepper({ volunteer }: { volunteer: VolunteerCardData }) {
  return (
    <div className="w-full max-w-sm bg-black/20 backdrop-blur-xl border border-white/10 rounded-[2rem] p-5 text-right shadow-2xl relative overflow-hidden">
      ...
      {/* التايم لاين التفاعلي مع الخط الرابط العمودي المحسن */}
      <div className="space-y-6 relative pr-2">
        
        {/* الخط الرابط البصري الذكي */}
        <div className="absolute right-[11px] top-3 bottom-3 w-[2px] bg-white/20 pointer-events-none"></div>
        
        {/* الخطوة 1: تم استلام البيانات */}
        <div className="flex flex-row-reverse items-start gap-3 relative z-10">
          <div className="w-6 h-6 rounded-full bg-emerald-500 flex items-center justify-center shrink-0 shadow-md shadow-emerald-900/50 border-2 border-white/80">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-emerald-300 font-bold mt-0.5">تم حفظ استمارة الحصر الميداني بنجاح واطمئنان</p>
          </div>
        </div>

        {/* الخطوة 2: المراجعة والتسكين */}
        <div className="flex flex-row-reverse items-start gap-3 relative z-10">
          <div className="w-6 h-6 rounded-full bg-red-500 flex items-center justify-center shrink-0 shadow-lg shadow-red-900 border-2 border-white/80 animate-pulse">
            <div className="w-1.5 h-1.5 bg-white rounded-full"></div>
          </div>
          <div className="flex-1">
            <h3 className="font-black text-[13px] text-white leading-tight">المراجعة والتسكين الميداني</h3>
            <p className="text-[11px] text-red-100/80 font-medium mt-1 leading-relaxed">
              ملفك الآن بطرف اللجنة المختصة لمراجعته وتسكينك رسمياً في وحدتك الميدانية.
            </p>
          </div>
        </div>

        {/* الخطوة 3: صدور الكارنيه */}
        <div className="flex flex-row-reverse items-start gap-3 relative z-10 opacity-50">
          <div className="w-6 h-6 rounded-full bg-white/10 flex items-center justify-center shrink-0 border-2 border-white/30"></div>
          <div className="flex-1">
            <h3 className="font-bold text-[13px] text-white/70 leading-tight">إصدار البطاقة الرقمية الرسمية</h3>
            <p className="text-[10px] text-white/50 font-medium mt-0.5">سيتم تفعيل وبناء الكارنيه فور صدور الاعتماد الإداري.</p>
          </div>
        </div>
      </div>

      {/* بطاقة ملخص مريحة تحت التتبع */}
      <div className="bg-white/5 border border-white/10 rounded-xl p-3.5 mt-5 flex flex-row-reverse items-center gap-3">
        <div className="w-12 h-12 rounded-lg border border-white/20 shadow-sm overflow-hidden shrink-0 bg-white flex items-center justify-center">
          {volunteer.photoUrl ? (
            <img src={volunteer.photoUrl} className="w-full h-full object-cover" alt="Review Avatar" />
          ) : (
            <div className="w-full h-full bg-[#991b1b] text-white flex items-center justify-center font-black text-xs">
              {getInitials(volunteer.fullName)}
            </div>
          )}
        </div>
        <div className="flex-1 text-right overflow-hidden">
          <p className="text-[13px] font-black text-white truncate leading-tight">{volunteer.fullName}</p>
          <p className="text-[10px] font-bold text-red-200 mt-1 bg-red-900/50 px-2.5 py-0.5 rounded-md inline-block">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#6b0714] via-[#800c19] to-[#4a020b] py-8 px-4 flex flex-col items-center justify-center space-y-5 select-none" dir="rtl" style={{ fontFamily: "'Cairo', sans-serif" }}>
      ...
      <div className="text-center space-y-2 max-w-md w-full flex flex-col items-center">
        ...
        {isApproved ? (
          <>
            <h1 className="text-xl font-black text-white tracking-tight">تهانينا، تم اعتماد هويتك الميدانية!</h1>
          ...
          <>
            ...
            <p className="text-xs text-red-100/70 font-bold max-w-xs">جاري تجهيز وتسكين ملفك الآن في السجلات الرسمية.</p>
      ...
      {/* نافذة المودال الفاخرة المخصصة للإرشادات */}
